...
namespace NBitcoin.Altcoins
{
    public class HeavyHash 
    {
		...
		public ulong[] Multiply(ulong[,] matrix, byte[] arrayInput) { 
			ulong[] product = new ulong[Obtc.RANK];
			
			for (int matrix_row = 0; matrix_row < Obtc.RANK; matrix_row++) {   
				for (int matrix_col = 0; matrix_col < Obtc.RANK; matrix_col++) {  
					product[matrix_row] +=   
